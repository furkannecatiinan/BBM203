...
void apply_mean_filter(const char* input_image, int kernel_size) {
    ...
    std::string output_filename = "F_mean_filtered_" + remove_extension(input_image) + "_" +
                                  std::to_string(kernel_size) + "x" + std::to_string(kernel_size) + ".png";
...
void apply_gaussian_smoothing(const char* input_image, int kernel_size, double sigma) {
    ...
    // String stream for formatting the sigma value
    std::stringstream sigmaStream;
    sigmaStream << std::fixed << std::setprecision(0) << sigma;  // Set to 2 decimal places

    std::string output_filename = "F_gaussian_filtered_" + remove_extension(input_image) + "_" +
                                  std::to_string(kernel_size) + "x" + std::to_string(kernel_size) + "_" +
                                  sigmaStream.str() + ".png";
...
void apply_unsharp_mask(const char* input_image, int kernel_size, double amount) {
    ...
    std::string output_filename = "F_unsharp_filtered_" + remove_extension(input_image) + "_" +
                                  std::to_string(kernel_size) + "x" + std::to_string(kernel_size) + "_" +
                                  std::to_string(amount) + ".png";
    img.save_to_file(output_filename.c_str());
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        else if (operation == "test_image") {
            if (argc < 3) throw std::invalid_argument("Usage: clearvision test_image <img>");
            GrayscaleImage img(argv[2]);
            std::cout << "Görüntü pikselleri: " << std::endl;
            img.print_image();
            return 0;
        }
