...
public:
    ...
    static int calculateMinMapDepthAccess(int playerDepth, int totalShaperTreeHeight, int totalMapDepth);
    ...
    // Yeni eklenecek işlevler:
    MapNode* findMin(MapNode* node); // Alt ağacın minimumunu bulur
    MapNode* balance(MapNode* node); // AVL ağacını dengeler

    void collectIslesInOrder(MapNode* node, std::vector<std::string>& isleNames);

    void logNodeDetails(MapNode* node, const std::string& message);
